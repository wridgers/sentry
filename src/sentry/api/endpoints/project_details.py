...
class ProjectAdminSerializer(serializers.Serializer):
    ...
    securityToken = serializers.RegexField(r'^[-a-zA-Z0-9+/= ]{1,255}$', max_length=255,
                                           required=False)
    securityTokenHeader = serializers.RegexField(r'^[a-zA-Z0-9-]{1,20}$', max_length=20,
                                                 required=False)
...
class ProjectDetailsEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project):
        ...
        data['securityTokenHeader'] = project.get_option('sentry:token_header', 'X-Sentry-Token')
        data.update({
    ...
    def put(self, request, project):
        ...
        data.update({
            ...
            'securityTokenHeader': project.get_option('sentry:token_header', 'X-Sentry-Troken'),
        })
